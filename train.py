...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    try:
        create_experiment('YOLOExperiment')
    except:
        get_experiment_id('YOLOExperiment')
    set_tracking_uri('http://localhost:5000')

    with mlflow.start_run():
        mlflow.set_experiment('YOLOExperiment')
    ...
    mlflow.log_params({"nominal_batch_size":nbs})
    accumulate = max(round(nbs / batch_size), 1)  # accumulate loss before optimizing
    hyp['weight_decay'] *= batch_size * accumulate / nbs  # scale weight_decay
    mlflow.log_params({"weight_decay":hyp['weight_decay']})
...
#             callbacks.run('on_train_epoch_end', epoch=epoch)
            ...
            temp_results = {"Precision":results[0],
                       "Recall":results[1],
                       "mAP@.5":results[2],
                       "mAP@.5-.95":results[3]
            }
            mlflow.log_metric(temp_results)
